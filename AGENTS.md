...
- `src/routes/mcp.ts` — MCP server for Codex integration (28 tools)
...
Use Codex (or Codex Desktop with MCP support) to connect directly to the `/mcp` HTTP server.
...
Tools return structured JSON using MCP `content: [{ type: "json", json: ... }]` where applicable, enabling Codex to consume results without text parsing.
