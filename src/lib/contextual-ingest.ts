...
import { contextualTasksClient } from './integrations';
...
export interface ContextualIngestDeps {
  getContextualDb?: (env: Env) => NeonQueryFunction<false, false> | null;
  fetchContextualCandidates?: typeof fetchContextualCandidates;
  classifyCandidate?: typeof classifyCandidate;
  createGoal?: typeof createGoal;
  createPlan?: typeof createPlan;
  createContextualIngestIntentIdempotent?: typeof createContextualIngestIntentIdempotent;
  tasksClient?: typeof contextualTasksClient;
  now?: () => Date;
}

const DEFAULT_DEPS: Required<ContextualIngestDeps> = {
  getContextualDb,
  ...
  tasksClient: contextualTasksClient,
  now: () => new Date(),
};
...
export async function ingestContextual(
  ...
  for (const cand of candidates) {
    ...
    try {
      ...
      const cls = await d.classifyCandidate(env, cand);
      ...
      try {
        const goal = await d.createGoal(env, {
          ownerChittyId: owner,
        ...
        const plan = await d.createPlan(env, {
          goalId: goal.id,
          title: `Ingest contextual message ${cand.message_id}`,
          authoredBy: 'contextual-ingest',
        });
        const { created } = await d.createContextualIngestIntentIdempotent(env, {
          planId: plan.id,
          goalId: goal.id,
      ...
      const dueDate =
        cand.due_date ??
        new Date(d.now().getTime() + 30 * 86400000).toISOString().slice(0, 10);
