...
// ── Tasks ───────────────────────────────────────────────────
// Lightweight task writer for internal reconciliation tasks. This uses the
// command database directly so contextual ingest can raise a durable cc_tasks
// record without depending on an external task service contract.
...
export function tasksClient(env: Env) {
  try {
    const sql = getDb(env);
    return {
      createTask: async (payload: TaskCreatePayload): Promise<Record<string, unknown> | null> => {
        try {
          const externalId =
    ...
    console.error('[tasks] client unavailable:', err);
    return null;
  }
}
